...
const PendingOrg = ({ userData }) => {
  ...
  const searchResultsListStyle = {
    ...
    zIndex: "1000",
    top: "17rem",
    boxShadow: "0px 0px 10px 0px #0ca678",
  };
  ...
  return (
    <div className="user-webpage-table-container">
      <div className="user-cards-container">
        <h1 style={{ color: "#0ca678", marginBottom: "20px" }}>
        ...
        >
          <Dropdown>
            ...
            <Dropdown.Menu>
              <Dropdown.Item onClick={() => applyTypeFilter("All")}>
                All
              </Dropdown.Item>
              {initialData.map((user) => (
                <Dropdown.Item
                  key={user.id}
                  onClick={() => applyTypeFilter(user.orgType)}
                >
                  {user.orgType}
                </Dropdown.Item>
              ))}
          ...
          <Dropdown>
            ...
            <Dropdown.Menu>
              <Dropdown.Item onClick={() => applyAreaFilter("All")}>
                All
              </Dropdown.Item>
              {initialData.map((user) => (
                <Dropdown.Item
                  key={user.id}
                  onClick={() => applyAreaFilter(user.orgArea)}
                >
                  {user.orgArea}
                </Dropdown.Item>
              ))}
          ...
          <Dropdown>
            ...
            <Dropdown.Menu>
              <Dropdown.Item onClick={() => applyGovFilter("All")}>
                All
              </Dropdown.Item>
              {initialData.map((user) => (
                <Dropdown.Item
                  key={user.id}
                  onClick={() => applyGovFilter(user.orgGov)}
                >
                  {user.orgGov}
                </Dropdown.Item>
              ))}
        ...
        {accept && (
          <Alert variant="success" style={{ fontWeight: "bold" }}>
            {acceptedOrg}'s Request Accepted
          </Alert>
        )}
        {reject && (
          <Alert variant="danger" style={{ fontWeight: "bold" }}>
            {rejectedOrg}'s Request Rejected
          </Alert>
        )}
        {initialData.map((user) => (
          <Accordion className="user-card" key={user.id}>
                ...
                <Row>
                  ...
                  <Col>
                    ...
                    <h6 style={textStyling}>
                      Name: {user.repFirstName} {user.repLastName}
                    </h6>
                  ...
                  <Col>
                    ...
                    >
                      <MdDownloadForOffline /> Documents
                    </button>
                  </Col>
                </Row>
                <Row
            ...
            <Modal
              scrollable="false"
              show={show[user.id] || false}
              onHide={handleClose}
            >
              <Modal.Header closeButton>
                <Modal.Title style={titleStyling}>
                  {user.orgName}'s Documents
                </Modal.Title>
              </Modal.Header>
              <Modal.Body>
                <iframe
